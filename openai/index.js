/**
 * @maitask/openai
 * OpenAI chat integration using Chat Completions API.
 *
...
async function execute(input = {}, options = {}, context = {}) {
  try {
    ...
    if (!response.ok) {
      return buildApiError('OPENAI_API_ERROR', 'OpenAI API request failed', response, cfg);
    }

    if (cfg.stream) {
      const streamResult = await parseOpenAIStream(response);
      return buildSuccess(
        {
    ...
    return buildError('OPENAI_API_ERROR', error, {
      provider: 'openai'
    });
  }
}
...
function buildConfig(input, options, context) {
  const apiKey =
    options.apiKey || options.api_key || input.apiKey || input.api_key || context?.secrets?.OPENAI_API_KEY;
  if (!apiKey) {
    throw new Error('OpenAI API key is required. Provide via options.apiKey or context.secrets.OPENAI_API_KEY');
  }
